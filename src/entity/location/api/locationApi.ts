    ...
    endpoints: (build) => ({
        ...
        >({
            queryFn: async (payload, _baseQuery, _extraConfig, fetchWithBQ) => {
                ...
                const locationWeather = await fetchWithBQ({
                    url: '/weather',
                    params: queryParams,
                })

                if (locationWeather.error) {
                    return { error: locationWeather.error as FetchBaseQueryError }
                }

                const normalLocationWeather = mapWeatherProps(
                    locationWeather.data as IWeatherAPI,
                )

                const { coordinates } = normalLocationWeather
